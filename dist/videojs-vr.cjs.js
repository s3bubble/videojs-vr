...
var DeviceOrientationControls = function DeviceOrientationControls(object) {
  ...
  var setObjectQuaternion = function () {
    var zee = new THREE$1.Vector3(0, 0, 1);
    var euler = new THREE$1.Euler();
    var q0 = new THREE$1.Quaternion();
    var q1 = new THREE$1.Quaternion(-Math.sqrt(0.5), 0, 0, Math.sqrt(0.5)); // - PI/2 around the x-axis
  ...
  this.update = function () {
    ...
    if (device) {
      var alpha = device.alpha ? THREE$1.Math.degToRad(device.alpha) + scope.alphaOffset : 0; // Z

      var beta = device.beta ? THREE$1.Math.degToRad(device.beta) : 0; // X'

      var gamma = device.gamma ? THREE$1.Math.degToRad(device.gamma) : 0; // Y''

      var orient = scope.screenOrientation ? THREE$1.Math.degToRad(scope.screenOrientation) : 0; // O
...
function Quat2Angle(x, y, z, w) {
  ...
  if (test > 0.499) {
    ...
    var _roll = 0;
    return new THREE$1.Vector3(_pitch, _roll, _yaw);
  ...
  if (test < -0.499) {
    ...
    var _roll2 = 0;
    return new THREE$1.Vector3(_pitch2, _roll2, _yaw2);
  }
  ...
  return new THREE$1.Vector3(pitch, roll, yaw);
}
